/*
  # Correction des évaluations référent

  1. Modifications
    - Ajout d'une fonction pour réinitialiser les évaluations bloquées
    - Mise à jour des statuts d'évaluation pour permettre aux référents d'évaluer
  2. Sécurité
...
BEGIN
  ...
  IF NEW.auto_evaluation IS NOT NULL AND NEW.auto_evaluation != '{}' AND NEW.statut = 'brouillon' THEN
    NEW.statut := 'soumise';
  END IF;
