/*
  # Add referent_projet role and update permissions

  1. New Role
    - Add 'referent_projet' to user_role enum
  2. Helper Functions
    - Create functions to check if user is referent_projet
    - Create function to check if user is project referent
    - Create function to check if user is admin or referent
  3. Policy Updates
    - Update project update policies
    - Update evaluation policies
    - Update project collaborator policies
  4. Fixes
    - Add function to reset stuck evaluations
*/
...
FOR UPDATE
TO authenticated
USING ((auth.uid() = auteur_id) OR (auth.uid() = referent_projet_id) OR is_admin() OR is_referent_projet());
...
USING (
  EXISTS (
    ...
    AND (p.referent_projet_id = auth.uid() OR p.auteur_id = auth.uid() OR is_admin() OR is_referent_projet())
  )
)
WITH CHECK (
  EXISTS (
...
USING (
  EXISTS (
    ...
    AND (p.auteur_id = auth.uid() OR p.referent_projet_id = auth.uid() OR is_admin() OR is_referent_projet())
  )
)
WITH CHECK (
  EXISTS (
