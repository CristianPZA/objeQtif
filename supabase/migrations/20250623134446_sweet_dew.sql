/*
  # Schéma complet objeQtifs

  ## 1. Tables principales
  - `user_profiles` - Profils utilisateurs multi-rôles
  - `fiches_annuelles` - Fiches d'évaluation annuelle
  - `fiches_projets` - Fiches de projets
  - `notifications` - Système de notifications
  - `historique_versions` - Historique versionné avec diff
  - `archives` - Archives des fiches (non supprimables)
  - `audit_logs` - Logs d'audit pour traçabilité

  ## 2. Sécurité et contraintes
  - RLS activé sur toutes les tables
  - Traçabilité complète des accès/modifications
  - Non-suppression des fiches archivées
  - Droit à l'oubli limité au compte utilisateur
  - Versioning automatique avec diff avant/après
...
-- Types de fiches
DO $$ BEGIN
  CREATE TYPE fiche_type AS ENUM (
    'annuelle',
    'projet',
    'evaluation'
  );
...
RETURNS void AS $$
BEGIN
  -- Cette fonction peut être appelée pour créer des données de test
  -- Elle sera implémentée selon les besoins
  RAISE NOTICE 'Test data creation function ready';
END;
$$ LANGUAGE plpgsql;